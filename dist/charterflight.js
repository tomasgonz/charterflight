...
class LineChart
{
  ...
  Draw(el)
  {
    ...
    const color$$1 = ordinal(category10);
    ...
    color$$1.domain(keys(data[0]).filter(key => key == "entity"));

    color$$1.domain(varNames);

    x.domain([min(data, ({values: values$$1}) => min(values$$1, ({date: date$$1}) => date$$1)),
      max(data, ({values: values$$1}) => max(values$$1, ({date: date$$1}) => date$$1))
    ]);

    y.domain([min(data, ({values: values$$1}) => min(values$$1, ({value}) => value)), max(data, ({values: values$$1}) => max(values$$1, ({value}) => value))]);
  ...
  // to avoid issues during event handler
    entities.append("svg:path")
      ...
      if (_self.LegendPlaceHolder !== "")
      {
        ...
        .append(`<div class="legend-item" id="legend-${d.key.sanitize()}" style="color:${color$$1(d.key)}">${d.key}</div>`);

        // Highlight line when hovering over legend

        select(`#legend-${d.key.sanitize()}`).on("mouseover", () => {

          let currClass = select(`#${d.key.sanitize()}`).attr("class");
          select(`#${d.key.sanitize()}`).attr("class", `${currClass} current`);

          currClass = select(`#legend-${d.key.sanitize()}`).attr("class");
          select(`#legend-${d.key.sanitize()}`).style('background-color', color$$1(d.key));
          select(`#legend-${d.key.sanitize()}`).style('color', "#fff");
        ...
        select(`#legend-${d.key.sanitize()}`).on("mouseout", () => {
          let currClass = select(`#${d.key.sanitize()}`).attr("class");
          let prevClass = currClass.substring(0, currClass.length - 8);
          select(`#${d.key.sanitize()}`).attr("class", prevClass);

          currClass = select(`#legend-${d.key.sanitize()}`).attr("class");
          prevClass = currClass.substring(0, currClass.length - 14);
          select(`#legend-${d.key.sanitize()}`).style('background-color', "#fff" );
          select(`#legend-${d.key.sanitize()}`).style('color', color$$1(d.key));
      ...
      .attr("d", ({values: values$$1}) => line$$1(values$$1))
      .on("mouseover", d => {
        let currClass = select(`#${d.key.sanitize()}`).attr("class");
        select(`#${d.key.sanitize()}`).attr("class", `${currClass} current`);
      ...
      .on("mouseout", ({key}) => {
      let currClass = select(`#${key.sanitize()}`).attr("class");
      let prevClass = currClass.substring(0, currClass.length - 8);
      select(`#${key.sanitize()}`).attr("class", prevClass);

      if (_self.LegendPlaceHolder !== "")
      {
        currClass = select(`#legend-${key.sanitize()}`).attr("class");
        prevClass = currClass.substring(0, currClass.length - 14);
        select(`#legend-${key.sanitize()}`).style('background-color', "#fff" );
        select(`#legend-${key.sanitize()}`).style('color', color$$1(key));
      }
    })
      .style("stroke", ({key}) => color$$1(key));
    ...
    entities.append("g").selectAll("circle")
      ...
      .append("circle")
      .attr("r", 3)
      .attr("cx", ({date: date$$1}) => x(date$$1))
      .attr("cy", ({value}) => y(value))
      .style("fill", ({entity}) => color$$1(entity))
      .attr("stroke", "none")
      .on("mouseover", ({entity, date: date$$1, value}) => {
        div.style("left", `${event.pageX}px`).style("top", `${event.pageY}px`);
        div.transition().duration(100).style("opacity", 100);
        div.html(`<p>entity: ${entity}<br />Date: ${date$$1.getFullYear()}<br/>Value: ${value}</p>`);
      })
      .on("mouseout", d => {
